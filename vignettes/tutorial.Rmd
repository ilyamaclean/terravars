...
```{r}
xy <- c(168750, 15050)
fp_down <- flowpath(dtm_filled, xy = xy, route = "mfd")
fp_up   <- flowpath(dtm_filled, xy = xy, route = "mfd", out = "upstream")
plot(c(fp_down, fp_up), col = rev(map.pal("viridis")), mar = c(2, 2, 2, 5))
```

![](images/water_img6.png)

By default (`route = "steepest"`), a single line is traced one step at a time, always moving to whichever neighbour is the steepest way down (or, for `out = "upstream"`, the steepest way up), and stopping at a pit or ridge, the edge of the DTM, or an `NA` cell. `route = "mfd"`, used above, spreads the water across every valid neighbour at once instead, weighted by slope in the same way as `flowacc()`'s own `method = "mfd"`, giving a continuous layer rather than a single line. Combined with `out`, this gives two rather different maps: `out = "downstream"` (the default direction) shows how the point's own water diffuses outward and downhill, while `out = "upstream"` instead shows the full upslope contributing area, graded by how much of each individual cell's own water actually reaches the point. This second case is useful when the direction of interest runs the other way - a pollutant turns up at a monitoring point, say, and the question is which part of the catchment it plausibly came from, and how strongly, rather than simply which cells lie somewhere upslope.
