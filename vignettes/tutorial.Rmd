...
`flowacc()` and the basin functions above describe the whole DTM at once. Sometimes the question is narrower than that: given a single point, where does the water there go, or where could it have come from? `flowpath()` answers this by tracing outward from one location rather than summarising the whole raster.
...
By default (`route = "steepest"`), a single line is traced one step at a time, always moving to whichever neighbour is the steepest way down (or, for `out = "upstream"`, used above, the steepest way up), and stopping at a pit or ridge, the edge of the DTM, or an `NA` cell. `out = "downstream"` (the default direction, also shown above) traces where the point's own water goes; `out = "upstream"` instead traces where it could have come from. The neighbourhood searched at each step follows the same `method` argument as `flowacc()`: `"d8"` (the default) considers all 8 surrounding cells, while `"d4"` restricts the search to the 4 orthogonal neighbours only.

A single traced line is not always the most useful answer, however - particularly upstream, where a whole area rather than one line is often what's actually wanted. Setting `route = "mfd"` spreads the water across every valid neighbour at once instead of committing to a single line, weighted by slope in the same way as `flowacc()`'s own `method = "mfd"`:
