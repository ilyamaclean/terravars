...
* [Hydrology](#hydrology)
  ...
  + [Tracing a flow path](#tracing-a-flow-path)
...
`terravars` is built around the `terra` package for raster data handling, so `terra` objects (`SpatRaster`) are the expected input and output format throughout this tutorial. `terravars` isn't on CRAN yet; install it directly from its [GitHub repository](https://github.com/ilyamaclean/terravars) using `remotes` (or `devtools`), then load it alongside `terra`:

```{r, message = FALSE}
# install.packages("remotes")
remotes::install_github("ilyamaclean/terravars")
...
Because `wind_shelter()` and `wind_altitude()` describe different processes, they are intended to be used together. Multiplying the two coefficients produces a single terrain-based adjustment that accounts for both directional shelter and topographic exposure.

```{r}
downscale_coef <- wa * ws[["wind_shelter_270"]]
plot(downscale_coef, main = "Combined wind downscaling coefficient (westerly)")
```
![](images/wind_altitude_img2.png)
...
```{r}
bsn_merged <- basinmerge(dtm_filled, bsn, boundary = 1)
...
Flow accumulation usually spans several orders of magnitude, so plotting on a log scale often reveals drainage patterns much more clearly than raw values. Three routing methods are available via `route`: `"d8"` (the default), which routes all flow to the single steepest downhill neighbour; `"mfd"`, which partitions flow among all downhill neighbours in proportion to slope, producing smoother, more diffuse drainage patterns; and `"dinf"`, which routes flow between the two neighbouring cells spanning the steepest downslope direction, with the split determined by how closely each aligns with that direction. This reduces the grid-aligned artefacts of D8 while avoiding the greater lateral dispersion produced by MFD. A `weight` raster can also be supplied, allowing accumulation of quantities such as rainfall volume instead of assuming each cell contributes equally. Supplying a basin raster via `bsn` restricts flow to within each basin, preventing it from crossing basin boundaries even where local gradients would otherwise allow it.

### Tracing a flow path

`flowacc()` and the basin functions described above summarise flow patterns across an entire DTM. Sometimes, however, the question is more specific: given a single location, where does the water flow, or where could it have come from? `flowpath()` answers this by tracing the flow from a single point rather than analysing the whole raster.


```{r}
xy <- c(168750, 15050)
pt <- vect(matrix(xy, ncol = 2), crs = crs(dtm_filled))

fp_down <- flowpath(dtm_filled, xy = xy)
fp_up   <- flowpath(dtm_filled, xy = xy, out = "upstream")

e <- ext(166000, 170000, 13000, 18000)
par(mfrow = c(1, 2))
plot(crop(fp_down, e), col = rev(map.pal("viridis")), main = "Downstream", mar = c(2, 2, 2, 5))
points(pt, pch = 21, bg = "red")
plot(crop(fp_up, e), col = rev(map.pal("viridis")), main = "Upstream", mar = c(2, 2, 2, 5))
points(pt, pch = 21, bg = "red")
```

![](images/water_img7.png)

By default `(route = "steepest")`, `flowpath()` follows a single route one cell at a time, always moving to the neighbouring cell with the steepest downhill gradient (or, when `out = "upstream"`, the steepest uphill gradient). Tracing stops when it reaches a pit or ridge, the edge of the DTM, or an NA cell. The default `out = "downstream"` traces the path that water from the chosen location would follow, whereas `out = "upstream"` traces the route by which water could have reached that point. The neighbourhood searched at each step is controlled by the `method` argument, as in `flowacc()`: `"d8"` (the default) considers all eight neighbouring cells, whereas `"d4"` restricts the search to the four orthogonal neighbours.

A single flow line is not always the most informative result, however, particularly when tracing upstream, where the contributing area is often of greater interest than any one route. Setting `route = "mfd"` distributes flow among all valid neighbouring cells, with weights proportional to slope using the same multiple-flow-direction algorithm as `flowacc(route = "mfd")`.

```{r}
fp_down_mfd <- flowpath(dtm_filled, xy = xy, route = "mfd")
fp_up_mfd   <- flowpath(dtm_filled, xy = xy, route = "mfd", out = "upstream")

e_mfd <- ext(166000, 171000, 12000, 18500)
par(mfrow = c(1, 2))
plot(crop(fp_down_mfd, e_mfd), col = rev(map.pal("viridis")), main = "Downstream", mar = c(2, 2, 2, 5))
points(pt, pch = 21, bg = "red")
plot(crop(fp_up_mfd, e_mfd), col = rev(map.pal("viridis")), main = "Upstream", mar = c(2, 2, 2, 5))
points(pt, pch = 21, bg = "red")
```

![](images/water_img6.png)

`out = "downstream"` now shows how water from the selected point diffuses outward and downhill, while `out = "upstream"` shows the full upslope contributing area, graded according to how much of each cell's own water reaches the selected point, rather than tracing only the single representative flow path shown above. This graded upstream view is particularly useful when the question runs in the opposite direction—for example, when a pollutant is detected at a monitoring point and the aim is to identify which parts of the catchment could have contributed to it, and by how much, rather than simply which cells lie somewhere upslope.

By default (`flat_source = TRUE`), the location given by `xy` is treated as the centre of a small water body rather than an infinitely small point. Thus any immediately adjacent DTM cells at the same elevation are included in the source area. Where the extent of a real water body is known, it can instead be supplied using `waterbody` (together with `water_value`, following the same convention as the corresponding arguments to `fillsinks()`). The traced flow path then starts from the entire water body rather than an approximation based on flat areas in the DTM.

The bundled `landcover` dataset (loaded earlier as `lcov` in [Preparing a DTM for hydrological analysis], where classes 13 and 14 represent Saltwater and Freshwater, respectively) make this straightforward to demonstrate using a real lake:


```{r}
xy_lake <- c(164850, 24850)
fp_lake <- flowpath(dtm_filled, xy = xy_lake, waterbody = lcov, water_value = c(13, 14),
                     route = "mfd", out = "upstream")
e_lake <- ext(162500, 169500, 22500, 28500)
plot(crop(fp_lake, e_lake), col = rev(map.pal("viridis")), main = "Catchment of a real lake")
points(vect(matrix(xy_lake, ncol = 2), crs = crs(dtm_filled)), pch = 21, bg = "red")
```

![](images/water_img8.png)

`xy_lake` here is simply a point somewhere within the lake. Left at its default, `flat_source` would only have picked up the handful of cells immediately touching that point at the same elevation; supplying `waterbody` instead identifies the lake's full extent from `landcover` (55 cells, in this case) and uses all of it as the source, so the catchment traced upstream is the one draining into the lake as a whole rather than into one arbitrary cell within it.
...
Every example so far has used the bundled Lizard peninsula data. For a new area, `dem_download()` and `landcover_download()` fetch a digital terrain model and a land-cover classification directly from public data sources, with no account, API key, or extra package required beyond `terravars` itself.

### Downloading land cover

`landcover_download()` fetches ESA WorldCover, a global 10 m resolution land-cover classification, matched to a template raster's own grid:

```{r}
lc <- landcover_download(dtm)   # watersplit = TRUE by default
plot(lc, main = "Downloaded land cover")
```

Categorical data like this is resampled onto the template grid using the modal ("most common") value in each output cell, rather than the interpolation used for continuous elevation data. ESA WorldCover's own classification doesn't distinguish sea from a large inland lake or river - both are class `80`, "Permanent water bodies". By default (`watersplit = TRUE`), `landcover_download()` splits this after fetching: any connected water body that touches the edge of the requested area is presumed connected to open water beyond it and kept as `80` (sea), while a water body fully enclosed within the interior becomes `81` (freshwater) instead. Setting `watersplit = FALSE` instead keeps ESA's original, undivided code `80` for all water:
...
With `watersplit = TRUE`, sea (`80`) and freshwater (`81`) show as two distinct classes; with `watersplit = FALSE`, every water body - sea or freshwater alike - is left as the single class `80`.

### Downloading a DTM

`dem_download()` takes a `SpatRaster` as a template - its extent, resolution and coordinate reference system define exactly what gets returned - and fetches elevation data to match, from the AWS "Terrain Tiles" public dataset.
...
The elevation data available from this source blends true land elevation with bathymetry, the latter generally at coarser resolution, which can produce oddities right at the coast. `dem_download()` doesn't fetch a land/sea mask itself; instead, its `mask` argument accepts any raster on the same grid to mask cells out of the result, with `mask_value` controlling which value(s) count as "mask this out" and `mask_out` controlling whether masked cells come back as `NA` (the default) or `0` (sea level). The land cover fetched above is a natural fit here - using `mask_value = 80` masks out the sea specifically, leaving any lake or river in the area untouched:
...
Passing `mask_value = c(80, 81)` instead masks out all water, matching ESA WorldCover's original, undivided class `80`. Keeping `mask` as a separate argument rather than fetching land cover internally means the same `lc` fetched above can be reused across as many `dem_download()` calls as needed, rather than repeating the fetch every time.
