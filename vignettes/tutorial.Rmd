...
`out = "downstream"` now shows how the point's own water diffuses outward and downhill, while `out = "upstream"` shows the full upslope contributing area, graded by how much of each individual cell's own water actually reaches the point, rather than the single representative line traced above. This graded version of `out = "upstream"` is useful when the direction of interest runs that way - a pollutant turns up at a monitoring point, say, and the question is which part of the catchment it plausibly came from, and how strongly, rather than simply which cells lie somewhere upslope.

`xy` is treated as sitting at the centre of a small water body rather than a single infinitely small point: by default, any DTM cell immediately adjacent to it at exactly the same elevation is included as well (`flat_source = TRUE`). Where a real water body is known, it can be supplied directly instead via `waterbody` (and `water_value`, following the same convention as `fillsinks()`'s own arguments above), so that the whole feature - not just an approximation from the DTM's own flat patches - is used as the starting point.

The bundled `landcover` data (already loaded as `lcov` in the [Preparing a DTM for hydrological analysis] section, where classes 13 and 14 represent Saltwater and Freshwater) makes this straightforward to demonstrate on an actual lake:
